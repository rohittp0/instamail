"""Anonymous public-profile harvester for the web_profile_info endpoint.

Uses curl_cffi with Chrome TLS impersonation (Instagram fingerprints plain
requests/httpx). Paces requests with a shared async rate limiter and backs off on
401/429, since anonymous access is aggressively throttled. An optional INSTAGRAM_SESSIONID
cookie markedly reduces 401s. Network deps are injected so the logic is unit-testable.
"""
...
class AsyncRateLimiter:
    """Min-interval limiter shared across concurrent fetches, with multiplicative jitter."""
